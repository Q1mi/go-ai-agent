...
// Provider 实现 OpenAI 兼容 Chat Completions。
...
func New(config Config) (*Provider, error) {
	...
	return &Provider{name: name, baseURL: baseURL, apiKey: apiKey, defaultModel: model, httpClient: client, toolCalling: config.ToolCalling}, nil
}
...
func toOpenAIMessages(messages []llm.Message) []openAIMessage {
	...
	for _, message := range messages {
		item := openAIMessage{Role: string(message.Role), Content: message.Content, ToolCallID: message.ToolCallID}
...
func parseChatResponse(body io.Reader) (*llm.ChatResponse, error) {
	...
	for _, call := range message.ToolCalls {
		...
		calls = append(calls, llm.ToolCall{ID: call.ID, Name: call.Function.Name, Args: args})
	}
	return &llm.ChatResponse{Content: message.Content, ToolCalls: calls}, nil
}
