...
const defaultSystemPrompt = "你是知识库问答助手。需要课程或文档资料时，请调用知识库检索工具，并在最终回答中标注来源。"
...
func (agent *Agent) run(ctx context.Context, goal string, out chan<- Event) {
	...
	if agent.registry == nil {
		agent.registry = tool.NewRegistry()
	}
	...
	for step := 0; step < agent.maxSteps; step++ {
		...
		if len(resp.ToolCalls) == 0 {
			...
			if answer == "" {
				emit(Event{Type: EventError, Text: "模型返回空响应"})
				return
			}
...
func (agent *Agent) callTool(ctx context.Context, name string, args json.RawMessage) string {
	...
	defer cancel()
	item, ok := agent.registry.Get(name)
	if !ok {
